...
check('100 €/mois à 8 % pendant 30 ans donnent 149 035,94 €', () => {
  const r = project({ initial: 0, monthly: 100, rate: 0.08, years: 30 });
  near(summarize(r.monthlyC, r.meta, 'exonere').finalCapital, 149035.94, 0.01, 'capital final');
});

check('viser 200 000 € demande 134,20 €/mois', () => {
  near(solveMonthly({ initial: 0, target: 200000, rate: 0.08, years: 30 }), 134.20, 0.01, 'effort mensuel');
});
